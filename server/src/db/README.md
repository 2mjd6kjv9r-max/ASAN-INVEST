...
Owned by the Database Specialist (`server/prisma/`, `server/src/db/`).
...
| Z-03 | Case `internal_status` is the only stored workflow status. Investor/stage views are mappings in `workflow_statuses` and `src/db/status-mapping.ts`. `stages` has no status column. |
...
Change this password before any shared or production deployment. 2FA is flagged enabled on the sysadmin row (NFR-02); the API still has to enforce the TOTP challenge.
